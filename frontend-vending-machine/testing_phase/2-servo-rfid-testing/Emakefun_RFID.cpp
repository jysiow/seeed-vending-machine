...
// On the Wio Terminal the external Grove I2C port lives on the Wire1 bus
// (SERCOM4), shared with the onboard accelerometer - an I2C scan shows the
// reader at 0x28 on Wire1. This library was written for the default `Wire`
// object, so remap every `Wire` reference below to `Wire1` for this board.
// (The #include above is unaffected because it is processed before this line.)
#define Wire Wire1
...
void MFRC522::PCD_Reset() {
	...
	// Section 8.8.2 in the datasheet says the oscillator start-up time is the start up time of the crystal + 37,74�s. Let us be generous: 50ms.
	delay(50);
	// Wait for the PowerDown bit in CommandReg to be cleared, but give up after
	// ~50ms. Without this bound, a reader that is absent or wired to the wrong
	// I2C bus returns 0xFF forever (bit 4 always set) and this loop hangs the
	// whole sketch before anything is drawn to the screen.
	byte tries = 0;
	while ((PCD_ReadRegister(CommandReg) & (1 << 4)) && tries < 50) {
		delay(1);
		tries++;
	}
} // End PCD_Reset()
...
String MFRC522::Read_Uid()
{
  ...
  {                 
    rfid_str = rfid_str + String(uid.uidByte[i], HEX);  //转为字符串
  }
  return rfid_str;
}
