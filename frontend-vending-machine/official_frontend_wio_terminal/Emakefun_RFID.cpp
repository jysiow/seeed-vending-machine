...
void MFRC522::PCD_Reset() {
	...
	// Section 8.8.2 in the datasheet says the oscillator start-up time is the start up time of the crystal + 37,74�s. Let us be generous: 50ms.
	delay(50);
	// Wait for the PowerDown bit in CommandReg to be cleared. Bounded so a missing
	// or unresponsive reader (whose I2C reads return 0xFF) can never spin here
	// forever and starve the rest of the firmware.
	byte resetGuard = 0;
	while ((PCD_ReadRegister(CommandReg) & (1<<4)) && (++resetGuard) < 20) {
		delay(5);   // up to ~100ms
	}
} // End PCD_Reset()
